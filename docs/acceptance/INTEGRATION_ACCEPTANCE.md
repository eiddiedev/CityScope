...
当前状态适合进入黑客松 Demo 打磨。确定性机制矩阵和 DeepSeek 新版多候选流程的完整真实配对验收均已完成；Stub 矩阵不替代模型质量结论。
...
| 独立接受边界 | 同一 seed 的 A/B 世界共享稳定私有画像；每个关键角色拥有风险容忍度、保留效用、最大让步和核心功能底线。 |
| BATNA 协调 | 双城方案分别与成都、重庆和企业的最佳替代方案比较；协调可成立、附条件接受、跳过或因不存在帕累托可行方案而僵局。 |
| 董事会统一选择 | 董事会同时比较已审计成都、重庆、双城、缩小规模和不落地选项；联合方案不再享有程序优先。 |
...
- 当前签名 Fixture：Baseline `DUAL_CITY`；四个干预分支分别为 `CHENGDU_LED / CHONGQING_LED / CHENGDU_LED / PROJECT_EXITED`，四类终局均由不同正式行动链成立。
...
- 2700 次离线敏感性：6 变量 × 9 值 × 50 seed，从与前端一致的开局 Checkpoint 运行；合法率 100%、0 未决、四结局全部可达。Stub 画像的默认诊断分布为双城协调 28%、成都主导 34%、重庆主导 28%、两城退出 10%；五个变量达到至少 30% 的配对变化率，其中人才、供应链、融资和可执行性四个变量的强干预达到至少 70%。该分布只证明机制没有塌缩，不作为真实 Agent 概率结论。
- DeepSeek 完整验收：20 条默认轨迹与 6 组 × 5 条配对干预，共 50 条真实轨迹全部完成。默认轨迹为双城协调 11/20（55%）、成都主导 5/20（25%）、重庆主导 4/20（20%）；全部轨迹合计为双城协调 13、成都主导 17、重庆主导 10、两城退出 10，四种正式终局均真实出现。人才增强组 5/5 成都主导，供应链增强组 5/5 重庆主导，融资和可执行性红线组均 5/5 退出；模型或合法缓存率 98.97%、fallback 1.03%、关键行动算法证据覆盖 100%、场景合法率 100%、无被拒行动修改世界。8 项自动验收全部通过，完整证据见 `fixtures/generated/deepseek-acceptance-report.json`。
...
- 页面回归：真实 API 使用 `deepseek-v4-flash`；离线回归验证谈判三列舞台、41 步黄金链路、结局复盘和默认折叠的技术证据，前端 22 项测试与生产构建通过。
...
1. 将 Live Fork 优化为读取已经验证的演示 Checkpoint，只现场续演干预分支，避免真实模型重复跑完整 Baseline。
2. 用真实采样替换单 LLM / 朴素群聊两组 evaluator fixture，并给出样本数和置信区间。
3. 完成 20 次连续路演彩排和录屏备份；确认 1280×720 投影下文字可读。
4. 若要部署，再增加持久化、SSE、鉴权、限流与多实例一致性；这些不属于当前 48 小时 Demo 必需项。
