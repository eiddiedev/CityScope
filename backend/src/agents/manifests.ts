...
const actors: ActorManifest[] = [
  agent({
    ...
    permissions: ["coordinate", "recommend", "pass"], forbidden: ["sign_policy", "transfer_budget"], goals: ["降低恶性竞争", "提高区域产业互补"], redLines: ["不得替地方签约", "不得指定企业结局"],
  ...
  cityAgent("chengdu_leader", "成都负责人 Agent", "chengdu", "regional_coordinator", ["sign_policy", "revise", "recommend", "pass"], [["rd_jobs", 0.25], ["headquarters", 0.25], ["fiscal_sustainability", 0.3], ["policy_credibility", 0.2]], ["形成成都正式政策包"], ["不得覆盖审计", "不得占用不存在资源"]),
  ...
  cityAgent("chongqing_leader", "重庆负责人 Agent", "chongqing", "regional_coordinator", ["sign_policy", "revise", "recommend", "pass"], [["manufacturing_output", 0.3], ["supply_chain", 0.25], ["fiscal_sustainability", 0.25], ["policy_credibility", 0.2]], ["形成重庆正式政策包"], ["不得覆盖审计", "不得占用不存在资源"]),
  agent({
  ...
  agent({
    agentId: "company_board", displayName: "星岚机器人董事会 Agent", organization: "company", role: "签发企业正式回应与投资决定", permissions: ["sign_company_response", "accept_policy", "withdraw_commitment", "exit_project", "pass"], forbidden: ["sign_policy", "decide_financing"], goals: ["平衡增长与偿付能力", "形成正式企业立场"], redLines: ["未收齐 CEO/CFO 意见不得决定", "不得接受未审计政策"], utility: [["enterprise_value", 0.4], ["liquidity_risk", 0.35, "minimize"], ["execution_probability", 0.25]], privateFactScopes: ["cash_runway", "order_quality"],
  }),
  agent({
  ...
  agent({
    agentId: "talent_sme", displayName: "人才与中小企业 Agent", organization: "stakeholder", role: "反馈人才吸引、住房、本地采购与供应链参与", permissions: ["publish_reaction", "pass"], forbidden: ["sign_policy", "sign_company_response", "transfer_budget"], goals: ["人才可持续流入", "本地中小企业参与"], redLines: ["住房压力过高时必须提示", "不得代表居民总体"], utility: [["talent_attraction", 0.3], ["sme_participation", 0.25], ["supply_chain_readiness", 0.25], ["housing_pressure", 0.2, "minimize"]], privateFactScopes: [],
  }),
  agent({
    agentId: "resident", displayName: "居民反应 Agent", organization: "stakeholder", role: "反馈就业收益、财政公平、土地能源交通压力与政府可信度", permissions: ["publish_reaction", "pass"], forbidden: ["sign_policy", "sign_company_response", "transfer_budget"], goals: ["公平分享就业收益", "控制公共资源压力"], redLines: ["财政公平恶化时必须提示", "不得伪造民调"], utility: [["jobs_benefit", 0.3], ["public_trust", 0.25], ["fiscal_fairness", 0.25], ["traffic_energy_pressure", 0.2, "minimize"]], privateFactScopes: [],
  }),
  service({
  ...
  service({
    agentId: "world_resource_service", displayName: "世界与资源规则服务", role: "确定性更新项目进度、资源与承诺触发器", permissions: ["advance_project", "pass"], forbidden: ["negotiate", "sign_policy", "free_language"], privateFactScopes: [], responsibilities: ["resource ledger transitions", "commitment trigger evaluation", "project progress update"],
  }),
];
