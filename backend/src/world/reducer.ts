    ...
    case "ACCEPT_POLICY": {
      ...
      for (const term of policy.terms.filter((item) => item.type === "cash_support" && (item.amountMillionCny ?? 0) > 0)) {
        const commitment: Commitment = {
          commitmentId: deterministicId("commitment", policy.policyId, term.termId),
          sourcePolicyId: policy.policyId,
          payer: `${policy.cityId}_government`,
          beneficiary: "xinglan_robotics",
          amountMillionCny: term.amountMillionCny ?? 0,
          ...(term.trigger ? { trigger: term.trigger } : {}),
          ...(term.deadline ? { deadline: term.deadline } : {}),
          failureAction: term.failureAction ?? "cancel_payment",
          status: "approved",
          createdAtVersion: version,
          lastCauseId: action.actionId,
        };
...
function addDelta(deltas: StateDelta[], action: AgentAction, version: number, path: string, before: unknown, after: unknown): void {
  ...
  deltas.push({ deltaId: deterministicId("delta", action.actionId, path, version), causeId: action.actionId, path, before: clone(before), after: clone(after), actorId: action.actorId, worldVersion: version });
}
